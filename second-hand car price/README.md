...
说明:这是我的第一个比赛，下面是个人的一些pipeline理解，可能有很多不对之处，敬请批评。4.9 baseline.ipynb文件是我提交的最好的一次结果，MAE=573，实在是无力回天了。这两天也有笔试，耽搁了。最终排名395/2746。跟预想的有很大差距，打比赛真的不容易。今后还是要好好加油才是。
...
对于特征，我们一般将其分为类别特征或者数字特征，可以通过dtype是number or object来简单地辨别，但真实场景中此种方法肯定是不行的。因为有时提供的数据早已对本来应该是类别的特征数字化，所以我们就需要通过已有的先验知识以及常识来对特征进行分类，分为numeric_features与categorical_features。并对于categorical_features特征通过.nunique()来查看其分布情况，包括种类与个数等等。
  ...
  2.5.1 数字特征分析
   ...
   ### 3.4 构建训练和测试样本：
   通过特征工程部分，从而为接下来的建模调参部分准备好了数据，我们可以得到用于模型的训练与测试样本。
   ...
   ## 5、模型融合
   ### 5.1 定义多个模型参数
   定义LR,Ridge,Lasso,GBDT,XGB,LGB等模型，包括其参数设定，或者利用何种调参方法网格or贝叶斯。
 
   ### 5.2 切分数据集（Train,Val）进行模型训练，评价和预测
   分别将数据切分成x_train,x_val,y_train,y_val，x_test,y_test这三部分，也就是训练，验证以及测试这三个数据集。然后再将训练数据x与标签y代入各个模型进行训练，利用训练好的模型分别验证验证集和测试集，得到验证集的预测标签和测试集的预测标签。
   ### 5.3 模型融合
   5.3.1 简单加权融合：可以直接利用权重对其验证集or测试集结果进行简单的加权融合，得到最终的预测验证标签or预测测试标签，再最后与真实的验证标签or测试标签求MAE平均绝对误差。
   
   5.3.2 进行Starking融合:利用前面的某些个模型的进行第一层训练，然后利用各个模型的输出作为第二层的输入，则可以进行Starking融合。但要注意，进行starking融合时候，基于模型层面的融合最好不同模型类型要有一定的差异，因为用同种模型不同的参数的收益一般是比较小的。
   
   后来我跑的时候看的结果是，lgb与xgb结合确实是效果最好的两个方式。stacking方式，第一层gbdt+xgb+lgb，第二层lr，泛化能力不如前者。
